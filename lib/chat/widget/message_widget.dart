...
class MessageWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // 系统消息
    if (ChatUtils.isSystemMessage(message!.type!)) {
      return _buildSystemMessageWidget();
    }
    //
    if (message!.isSend()) {
      return _buildSendWidget(context);
    }
    //
    return _buildReceiveWidget(context);
  }
  ...
  Widget _buildSendWidget(BuildContext context) {
    ...
    // FIXME: 消息状态，待完善
    String status = '';
    if (message!.status == ChatConsts.MESSAGE_STATUS_SENDING) {
      status = '发送中';
    } else if (message!.status == ChatConsts.MESSAGE_STATUS_SUCCESS) {
      status = ''; // 发送成功
    } else if (message!.status == ChatConsts.MESSAGE_STATUS_DELIVERED) {
      status = '送达';
    } else if (message!.status == ChatConsts.MESSAGE_STATUS_READ) {
      status = '已读';
    } else if (message!.status == ChatConsts.MESSAGE_STATUS_ERROR) {
      status = '失败';
    }
    return Container(
          ...
          children: menuItems
              .map((item) => GestureDetector(
                    ...
                    onTap: () {
                      ...
                      if (value == 'copy') {
                        /// 把文本复制进入粘贴板
                        if (message.type == ChatConsts.MESSAGE_TYPE_TEXT) {
                          Clipboard.setData(
                              ClipboardData(text: message.content!.tr));
                        } else if (message.type ==
                            ChatConsts.MESSAGE_TYPE_IMAGE) {
                          Clipboard.setData(
                              ClipboardData(text: message.content!));
                        } else if (message.type ==
                            ChatConsts.MESSAGE_TYPE_VIDEO) {
                          Clipboard.setData(
                              ClipboardData(text: message.content!));
                        } else if (message.type ==
                            ChatConsts.MESSAGE_TYPE_FILE) {
                          Clipboard.setData(
                              ClipboardData(text: message.content!));
                        } else if (message.type ==
                            ChatConsts.MESSAGE_TYPE_AUDIO) {
                          Clipboard.setData(
                              ClipboardData(text: message.content!));
                        } else {
                          Clipboard.setData(
  ...
  Widget _buildSendContent(BuildContext context, Message message) {
    ...
    } else if (message.type == ChatConsts.MESSAGE_TYPE_IMAGE) {
      return InkWell(
        onTap: () {
          // 支持将图片保存到相册
          Navigator.push(
    ...
    } else if (message.type == ChatConsts.MESSAGE_TYPE_GOODS) {
    ...
    } else if (message.type == ChatConsts.MESSAGE_TYPE_VIDEO) {
      return InkWell(
        onTap: () {
          debugPrint('play video');
          Navigator.of(context).push(MaterialPageRoute(builder: (context) {
            return VideoPlayPage(videoUrl: message.content);
          }));
        },
        child: SizedBox(
          width: 100,
          height: 100,
          child: CachedNetworkImage(
            imageUrl: ChatConsts.VIDEO_PLAY,
            errorWidget: (context, url, error) => const Icon(Icons.error),
          ),
        ),
      );
          ...
          Row(
            ...
            children: <Widget>[
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // 昵称，TODO: 除了群聊之外，不需要显示昵称
                  // Container(
                  //   // color: Colors.grey,
                  //   margin: const EdgeInsets.only(bottom: 5),
                  //   child: SelectableText(
                  //     message!.user!.nickname!.tr,
                  //     style: const TextStyle(fontSize: 10),
                  //   ),
                  // ),
  ...
  Widget _buildReceivedContent(BuildContext context, Message message) {
    ...
    } else if (message.type == ChatConsts.MESSAGE_TYPE_ROBOT) {
      return Column(
        children: <Widget>[
          // Text(
          //   message.content ?? '',
    ...
    //   );
    // }
     else if (message.type == ChatConsts.MESSAGE_TYPE_FAQ) {
      Map<String, dynamic> jsonMap = json.decode(message.content!);
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
    ...
    } else if (message.type == ChatConsts.MESSAGE_TYPE_NOTICE) {
      // {
      ...
      return Column(
        ...
        children: <Widget>[
          Text(
            _parseJson(message.content.toString()),
            style: const TextStyle(fontSize: 12),
          ),
        ],
      );
    ...
    return Row(
      ...
      children: <Widget>[
        Container(
          ...
          child: Text(
             ChatUtils.shortTimeFormat(message!.createdAt!),
  ...
  Widget _buildAvatarWidget() {
    return SizedBox(
      ...
      height: 35,
      child: CachedNetworkImage(
        imageUrl: message!.user!.avatar!,// ?? BytedeskConstants.DEFAULT_AVATA,
        errorWidget: (context, url, error) => const Icon(Icons.error),
      ),
    );
  }
  ...
  String _parseJson(String jsonString) {
    ...
    try {
      ...
      var sb = StringBuffer();
      var length = jsonMap.keys.length;
      var count = 0;
      for (var key in jsonMap.keys) {
        sb.write("${key.tr}: ${jsonMap[key]}");
        if (count < length - 1) {
          sb.write("\n");
          count++;
        }
