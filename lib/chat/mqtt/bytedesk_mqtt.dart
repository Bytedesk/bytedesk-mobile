 ...
 * @LastEditTime: 2025-01-06 15:58:49
...
import 'package:im/chat/mqtt/events.dart';
...
class BytedeskMqtt {
  ...
  void connect() async {
    //
    if (!ChatUtils.isLogin()!) {
      debugPrint('i18n.auth.not.login'.tr);
      return;
    }
    //
    if (isConnected()) {
      debugPrint('i18n.mqtt.already.connected'.tr);
      return;
    }
    bytedeskEventBus
        .fire(ConnectionEventBus(ChatConsts.USER_STATUS_CONNECTING));
    //
    currentUid = SpUtil.getString(ChatConsts.uid);
    username = SpUtil.getString(ChatConsts.username);
    ...
    accessToken = SpUtil.getString(ChatConsts.accessToken);
    client = ChatUtils.getClient();
    deviceUid = SpUtil.getString(ChatConsts.deviceUid);
    clientId = "$currentUid/$client/$deviceUid";
    messageProvider = MessageProvider();
    debugPrint('mqttClient clientId: $clientId');
    ...
    //         'ws://127.0.0.1/websocket', clientId!, ChatConsts.mqttPort);
    //   } else {
    //     mqttClient = MqttBrowserClient.withPort(
    //         ChatConsts.webSocketWssUrl, clientId!, 443);
    //   }
    // } else {
    if (!ChatConsts.isDebug) {
      debugPrint('i18n.mqtt.websocket.connecting'.tr);
      mqttClient = MqttServerClient(ChatConsts.webSocketUrl, clientId!);
      ...
      // mqttClient.websocketProtocols = MqttClientChatConsts.protocolsSingleDefault;
    } else {
      debugPrint('i18n.mqtt.debug.connecting'.tr);
      mqttClient = MqttServerClient(ChatConsts.webSocketUrl, clientId!);
      // mqttClient?.port = ChatConsts.mqttPort;
      mqttClient?.useWebSocket = true;
      mqttClient?.port = 9885;
      // mqttClient?.secure = ChatConsts.isSecure;
    }
    // }
    ...
    // mqttClient.logging(on: ChatConsts.isDebug);
    ...
    final MqttConnectMessage connMessage = MqttConnectMessage()
        ...
        .authenticateAs(username, accessToken);
    ...
    mqttClient?.published?.listen((MqttPublishMessage messageBinary) {
      ...
      debugPrint('receive message uid: $uid');
      // 处理接收消息
      ...
      if (!message.isSend()) {
        // 接收的非自己发送消息
        switch (message.type) {
          case ChatConsts.MESSAGE_TYPE_READ:
          case ChatConsts.MESSAGE_TYPE_DELIVERED:
            // 回执消息
            updateMessageStatus(message);
            return;
          case ChatConsts.MESSAGE_TYPE_TYPING:
          case ChatConsts.MESSAGE_TYPE_PROCESSING:
            // 非自己发送的：正在输入
            handleTypingMessage(message);
            return;
          case ChatConsts.MESSAGE_TYPE_PREVIEW:
            // 非自己发送的：消息预知
            handlePreviewMessage(message);
            return;
          case ChatConsts.MESSAGE_TYPE_FAQ_UP:
          case ChatConsts.MESSAGE_TYPE_FAQ_DOWN:
          case ChatConsts.MESSAGE_TYPE_ROBOT_UP:
          case ChatConsts.MESSAGE_TYPE_ROBOT_DOWN:
          case ChatConsts.MESSAGE_TYPE_RATE_SUBMIT:
          case ChatConsts.MESSAGE_TYPE_RATE_CANCEL:
            // 访客提交评价或取消评价
            updateMessageStatus(message);
            return;
          case ChatConsts.MESSAGE_TYPE_STREAM:
            // handleTypingMessage(currentThread, thread, messageProtobuf.getType());
            break;
          case ChatConsts.MESSAGE_TYPE_TRANSFER:
            // 转接
            debugPrint("transfer message");
            // handleTransferMessage(message, thread);
            break;
          case ChatConsts.MESSAGE_TYPE_TRANSFER_ACCEPT:
            // 转接被接受
            debugPrint("transfer accept message");
            // handleTransferAcceptMessage(message, thread);
            return;
          case ChatConsts.MESSAGE_TYPE_TRANSFER_REJECT:
            // 转接被拒绝
          ...
          default:
            ...
            //
            if (ChatUtils.shouldSendReceipt(messageProto.type)) {
      ...
      //
      messageProvider?.insert(message, currentUid!);
      // 通知界面显示聊天记录
...
// FIXME: ld: library not found for -lvibration
  ...
  void sendTextMessage(String uid, String content, Thread currentThread) {
    publish(uid, content, ChatConsts.MESSAGE_TYPE_TEXT, currentThread);
  }

  void sendImageMessage(String uid, String content, Thread currentThread) {
    publish(uid, content, ChatConsts.MESSAGE_TYPE_IMAGE, currentThread);
  }

  void sendFileMessage(String uid, String content, Thread currentThread) {
    publish(uid, content, ChatConsts.MESSAGE_TYPE_FILE, currentThread);
  }

  void sendAudioMessage(String uid, String content, Thread currentThread) {
    publish(uid, content, ChatConsts.MESSAGE_TYPE_AUDIO, currentThread);
  }

  void sendVideoMessage(String uid, String content, Thread currentThread) {
    publish(uid, content, ChatConsts.MESSAGE_TYPE_VIDEO, currentThread);
  }

  void sendPreviewMessage(String previewContent, Thread currentThread) {}

  void sendRecallMessage(String uid, Thread currentThread) {}

  void sendReceiptReceivedMessage(String uid, Thread currentThread) {
    publish(BytedeskUuid.uuid(), uid, ChatConsts.MESSAGE_TYPE_DELIVERED,
        currentThread);
  }

  void sendReceiptReadMessage(String uid, Thread currentThread) {
    publish(
        BytedeskUuid.uuid(), uid, ChatConsts.MESSAGE_TYPE_READ, currentThread);
  }

  void publish(
      String messageUid, String content, String type, Thread currentThread) {
    debugPrint(
        'publish message content: $type, $content, ${currentThread.toJson()}');
    ...
    if (isConnected()) {
      ...
      if (currentThread.type == ChatConsts.THREAD_TYPE_AGENT ||
          currentThread.type == ChatConsts.THREAD_TYPE_WORKGROUP) {
        user.uid = agentJson.data!.uid!;
        user.nickname = agentJson.data!.nickname!;
        user.avatar = agentJson.data!.avatar!;
      } else {
        user.uid = currentUid!;
      ...
      message.type = type;
      message.status = ChatConsts.MESSAGE_STATUS_SENDING;
      message.createdAt = ChatUtils.formattedDateNow();
      message.client = ChatUtils.getClient();
      ...
      String? createdAt = ChatUtils.formattedDateNow();
      ...
      if (currentThread.type == ChatConsts.THREAD_TYPE_AGENT ||
          currentThread.type == ChatConsts.THREAD_TYPE_WORKGROUP) {
        userProtobuf.uid = agentJson.data!.uid!;
        userProtobuf.nickname = agentJson.data!.nickname!;
        userProtobuf.avatar = agentJson.data!.avatar!;
      }
      //
      var messageExtra = {
        orgUid: orgUid!,
...
// 断开长连接
  ...
  void _onDisconnected() {
    ...
    // eventbus发广播，通知长连接断开
    bytedeskEventBus
        .fire(ConnectionEventBus(ChatConsts.USER_STATUS_DISCONNECTED));
  ...
  void _onConnected() {
    ...
    bytedeskEventBus.fire(ConnectionEventBus(ChatConsts.USER_STATUS_CONNECTED));
  }
  ...
  void updateMessageSuccess(String uid) {
    messageProvider?.update(uid, ChatConsts.MESSAGE_STATUS_SUCCESS);
    //
    bytedeskEventBus.fire(
        ReceiveMessageReceiptEventBus(uid, ChatConsts.MESSAGE_STATUS_SUCCESS));
  }
