...
class FlutterPluginRecord {
  ...
  ///初始化init的回调
  final StreamController<bool> _responseInitController =
      StreamController.broadcast();

  Stream<bool> get responseFromInit => _responseInitController.stream;

  ///开始录制 停止录制的回调监听
  final StreamController<RecordResponse> _responseController =
      StreamController.broadcast();

  Stream<RecordResponse> get response => _responseController.stream;

  ///音量高低的回调
  final StreamController<RecordResponse> _responseAmplitudeController =
      StreamController.broadcast();
  ...
  ///播放状态监听
  final StreamController<PlayState> _responsePlayStateController =
      StreamController.broadcast();
